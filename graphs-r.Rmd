...
starwarsRec |> 
  ...
  geom_jitter(aes(x = as.numeric(species) - .15, color = species),
              show.legend = FALSE, size = .8, width = .1) +
  labs(x = "Species",
       y = "Height (cm)",
  ...
  coord_flip() +
  scale_fill_ie(palette = 'palette_categorical') +
  scale_color_ie(palette = 'palette_categorical') +
...
starwarsRec |> 
  ...
  labs(x = "Species",
       y = "Mass (kg)",
       title = "Distribution of Species Mass for Star Wars\nCharacters") +
  theme_minimal(base_size = 14) +
  coord_flip() +
  scale_fill_ie(palette = 'palette_categorical') +
  scale_color_ie(palette = 'palette_categorical') +
...
* Change the palettes for color and fill to one of the two other IE palettes, see https://github.com/jenineharris/iestylr
